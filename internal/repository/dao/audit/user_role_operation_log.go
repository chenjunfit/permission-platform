...
type UserRoleLogDAO interface {
	BatchCreate(ctx context.Context, log []UserRoleLog) error
}
type userRoleLogDAO struct {
	db *egorm.Component
}

func (u *userRoleLogDAO) BatchCreate(ctx context.Context, log []UserRoleLog) error {
	if len(log) == 0 {
		return nil
	}
	const batchSize = 100
	...
	for i := range log {
		log[i].Ctime, log[i].Utime = now, now
	}
	return u.db.WithContext(ctx).CreateInBatches(log, batchSize).Error
}
